...
// TODO: use ImmediateImageState here, add pipeline stage mask field to the image state structure
class CommandGenerateMipmaps {
    ...
    void prepare_destination_mip_level(Etna::CommandQueue* command_buffer, uint32_t mip_level, VkImageLayout old_layout, VkAccessFlags old_access_mask) {
...
//         if(state == desired_state) return;
        ...
        barrier.get_subresource_range().set_aspect_mask(m_aspect_flags);
        barrier.get_subresource_range().set_base_mip_level(mip_level);
        barrier.set_layouts(old_layout, m_target_layout);
        barrier.set_access_masks(old_access_mask, m_target_access_mask);
...
public:
    ...
    void perform(Etna::CommandQueue* command_buffer) {
        ...
        VkAccessFlags previous_access_flags = image_state.m_access_mask;
        VkImageLayout previous_layout = image_state.m_layout;

        for (uint32_t i = 1; i < mip_levels; i++) {
            prepare_source_mip_level(command_buffer, i - 1, previous_layout, previous_access_flags);
            prepare_destination_mip_level(command_buffer, i, original_layout, original_access_flags);
        ...
        image_state.m_access_mask = m_target_access_mask;
    }
